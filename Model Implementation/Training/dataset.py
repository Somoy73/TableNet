...
class ImageFolder(nn.Module):
    def __init__(self, df, isTrain=False, transform=None):
    ...
    def __getitem__(self, index):
        image_path, table_mask_path, column_mask_path = (
            self.df.iloc[index, 0],
            self.df.iloc[index, 1],
            self.df.iloc[index, 2],
        )
        image = np.array(Image.open(image_path))
        table_image = torch.FloatTensor(
            np.array(Image.open(table_mask_path)) / 255.0
        ).reshape(1, 1024, 1024)
        column_image = torch.FloatTensor(
            np.array(Image.open(column_mask_path)) / 255.0
        ).reshape(1, 1024, 1024)
