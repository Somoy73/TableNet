...
def get_data_loaders(data_path=DATAPATH):
    ...
    train_data, test_data = train_test_split(
        df, test_size=0.2, random_state=SEED, stratify=df.hasTable
    )
...
def write_summary(writer, tr_metrics, te_metrics, epoch):
    ...
    writer.add_scalar(
        "Column Precision/Train", tr_metrics["col_precision"], global_step=epoch
    )
    writer.add_scalar(
        "Column Precision/Test", te_metrics["col_precision"], global_step=epoch
    )
    writer.add_scalar(
        "Column Recall/Train", tr_metrics["col_recall"], global_step=epoch
    )
...
def get_TableMasks(test_image, model, transform=TRANSFORM, device=DEVICE):
    ...
    column_out = (
        column_out.cpu().detach().numpy().squeeze(0).transpose(1, 2, 0) > 0.5
    ).astype(int)
    # Return masks
...
def fixMasks(image, table_mask, column_mask):
    ...
    for x, y, w, h in table_bound_rect:
        ...
        # Get contours of the mask to get number of tables
        contours, column_heirarchy = cv2.findContours(
            column_mask_crop, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        # Get bounding box for the contour
