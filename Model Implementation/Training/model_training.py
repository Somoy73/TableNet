...
from path_constants import PROCESSED_DATA
from tablenet_model import TableNet
from model_loss import TableNetLoss
...
def train_on_epoch(data_loader, model, optimizer, loss, scaler, threshold=0.5):
    ...
    for batch_i, image_dict in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            ...
            i_table_loss, i_column_loss = loss(
                table_out, table_image, column_out, column_image
            )
...
def main():
    ...
    model = TableNet(
        encoder="densenet", use_pretrained_model=True, basemodel_requires_grad=True
    )
    ...
    if os.path.exists(checkpoint_name):
        last_epoch, train_metrics, test_metrics = load_checkpoint(
            torch.load(checkpoint_name), model
        )
        last_table_f1 = test_metrics["table_f1"] - abs(
            test_metrics["table_f1"] - train_metrics["table_f1"]
        )
    ...
    writer = SummaryWriter(
        f"{PROCESSED_DATA}/runs/TableNet/densenet/configuration_4_batch_{BATCH_SIZE}_learningrate_{LEARNING_RATE}_encoder_train"
    )
    # For early stopping
    i = 0
    ...
    for epoch in range(last_epoch + 1, EPOCHS):
        print("=" * 30)
        start = time.time()
        train_metrics = train_on_epoch(
            train_loader, model, optimizer, loss, scaler, threshold=0.5
        )
        ...
        # test_f1 - abs(test_f1 , train_f1) > prev
        if last_table_f1 < test_metrics["table_f1"] - abs(
            test_metrics["table_f1"] - train_metrics["table_f1"]
        ):
            last_table_f1 = test_metrics["table_f1"] - abs(
                test_metrics["table_f1"] - train_metrics["table_f1"]
